import { useState } from "react";
import { useAuth } from "@/contexts/AuthContext";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { getAccessCode } from "@/lib/firestore";
import { Lock } from "lucide-react";

const LoginPage = () => {
  const { signInWithGoogle } = useAuth();
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const [codeInput, setCodeInput] = useState("");
  const [codeVerified, setCodeVerified] = useState(false);
  const [codeLoading, setCodeLoading] = useState(false);
  const [codeError, setCodeError] = useState<string | null>(null);

  const handleVerifyCode = async (e: React.FormEvent) => {
    e.preventDefault();
    setCodeLoading(true);
    setCodeError(null);
    try {
      const validCode = await getAccessCode();
      if (validCode && codeInput.trim() === validCode.trim()) {
        setCodeVerified(true);
      } else {
        setCodeError("Code d'accès incorrect. Veuillez réessayer.");
      }
    } catch {
      setCodeError("Erreur de vérification. Réessayez.");
    } finally {
      setCodeLoading(false);
    }
  };

  const handleGoogleLogin = async () => {
    setLoading(true);
    setError(null);
    try {
      await signInWithGoogle();
    } catch {
      setError("Erreur de connexion. Réessayez.");
      setLoading(false);
    }
  };

  return (
    <div className="min-h-screen flex items-center justify-center bg-background px-4">
      <div className="w-full max-w-md text-center">
        <div className="mb-8">
          <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-primary/10 mb-6">
            <svg className="h-8 w-8 text-primary" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <path d="M15 3h4a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2h-4" />
              <polyline points="10 17 15 12 10 7" />
              <line x1="15" y1="12" x2="3" y2="12" />
            </svg>
          </div>
          <h1 className="text-3xl font-bold text-foreground mb-2">Media School</h1>
          <p className="text-muted-foreground">Accède à toutes tes formations</p>
        </div>

        {!codeVerified ? (
          <form onSubmit={handleVerifyCode} className="flex flex-col gap-4">
            <div className="flex flex-col gap-2 text-left">
              <div className="flex items-center gap-2 mb-1">
                <Lock className="h-4 w-4 text-muted-foreground" />
                <span className="text-sm font-medium text-foreground">Code d'accès requis</span>
              </div>
              <Input
                type="password"
                placeholder="Entrez le code d'accès"
                value={codeInput}
                onChange={(e) => setCodeInput(e.target.value)}
                className="h-12 text-base"
                autoFocus
              />
              {codeError && <p className="text-destructive text-sm">{codeError}</p>}
            </div>
            <Button
              type="submit"
              disabled={codeLoading || !codeInput.trim()}
              className="w-full h-12 text-base font-semibold"
              size="lg"
            >
              {codeLoading ? "Vérification..." : "Accéder"}
            </Button>
          </form>
        ) : (
          <>
            <p className="text-sm text-muted-foreground mb-4">Code validé. Connecte-toi pour continuer.</p>
            <Button
              onClick={handleGoogleLogin}
              disabled={loading}
              className="w-full h-12 text-base font-semibold gap-3"
              size="lg"
            >
              <svg className="h-5 w-5" viewBox="0 0 24 24">
                <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="#4285F4" />
                <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" />
                <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05" />
                <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335" />
              </svg>
              {loading ? "Connexion..." : "Continuer avec Google"}
            </Button>
            {error && <p className="text-destructive text-sm mt-4">{error}</p>}
          </>
        )}

        <div className="mt-10 flex flex-col items-center gap-0.5">
          <p className="text-xs text-muted-foreground/60">© Media Compassion Bruxelles</p>
          <p className="text-xs text-muted-foreground/40">Powered by <span className="font-medium text-muted-foreground/60">Martinez Muzela</span></p>
        </div>
      </div>
    </div>
  );
};

export default LoginPage;
